...
# K  = log u - log(2-u)
# K' = 2u' / [u(2-u)]
# K''= (N'D - ND') / D^2  where N=2u', D=u(2-u), N'=2u'', D'=2(1-u)u'
...
def cgf(double t, double nu):
    ...
    if t >= _t_upper(nu): raise ValueError(f"t must be < t*={_t_upper(nu):.6f}")
    return _cgf(t, nu)
...
def find_saddlepoint(double x, double nu):
    ...
    if x <= 0.0: raise ValueError("x must be > 0.")
    cdef double t = _find_saddlepoint(x, nu)
    if isnan(t): raise RuntimeError(f"Newton-Raphson failed for x={x}, nu={nu}")
    return t
...
def fstar_sp(double x, double nu):
    ...
    if isnan(v): raise RuntimeError(f"fstar_sp failed for x={x}, nu={nu}")
    return v
...
def Fstar_lr(double x, double nu):
    ...
    if isnan(v): raise RuntimeError(f"Fstar_lr failed for x={x}, nu={nu}")
    return v

# ── Two-pathway mixture density h(x; nu, p) ───────────────────────────────────
