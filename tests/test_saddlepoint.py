...
def check(label, val, ref, tol, show=True):
    ...
    if show:
        ...
        print(f"  {status}  {label:<55s}  approx={val:.8e}  ref={ref:.8e}  rel_err={err:.2e}")
    return ok
...
def test_cgf_moments():
    ...
    for nu in [1.0, 2.5, 5.0, 10.0]:
        ...
        ok2 = check(f"nu={nu}: K''(0) == (nu+1)/(2nu) = {var_ref:.4f}", kd2, var_ref, 1e-9)
...
def test_loglik_smoke():
    ...
    all_gaps    = np.clip(rng.gamma(nu, 1.0/(2*nu), size=int(n_gaps_list.sum())), 0.01, None)
...
def test_loglik_peak():
    ...
    all_gaps     = np.clip(rng.gamma(shape=nu_true, scale=1.0/(2*nu_true),
                                     size=int(n_gaps_list.sum())), 1e-6, None)
    ...
    for nu_test, p_test in [(1.0, 0.5), (2.0, 0.3), (10.0, 0.0), (0.5, 0.9)]:
        ...
        ok = ll_true >= ll_other
        status = PASS if ok else FAIL
        print(f"  {status}  ll(true)={ll_true:.2f} >= ll({nu_test},{p_test})={ll_other:.2f}")
        all_ok = all_ok and ok
    return all_ok
...
if __name__ == "__main__":
    tests = [
        test_cgf_moments,
        test_saddlepoint_solver,
        test_nu1_exponential,
        test_density_accuracy,
        test_cdf_accuracy,
        test_density_integrates_to_one,
        test_array_functions,
        test_h_mixture_limits,
        test_loglik_smoke,
        test_loglik_peak,
    ]
